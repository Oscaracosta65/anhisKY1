 ...
 *
 * Game: KY1 — Kentucky
 * Main balls : 4  (columns: first, second, third, fourth)  range 01–35
 * Bonus ball  : 1  (column:  fifth)                         range 01–25
 *
...
/* ---- game structure (derived from target's own code) ------------------- */
...
if (isset($stateName, $gName) && $stateName !== '' && $gName !== '') {
    ...
    $doc->setMetaData('description', $stateName . ' ' . $gName . ' draw frequency, recency, active and quiet numbers, and full historical analysis. Transparent data-driven lottery intelligence on LottoExpert.net.');
}
...
function leGetPreviousOccurrenceDate(
    ...
    if ($isBonus) {
        $query->where($db->quoteName($bonusCol) . ' = ' . $db->quote($ball));
    } else {
        $parts = [];
...
[$bonusCounts, $bonusLastSeenIndex] = leInitRange($bonusRangeMin, $bonusRangeMax);
...
foreach ($rowsBonus as $idx => $row) {
    $ball = trim((string) ($row[$bonusBallCol] ?? ''));
    ...
    if ($bonusLastSeenIndex[$ball] === null) {
        $bonusLastSeenIndex[$ball] = (int) $idx;
    }
}
...
$bonusChartLabels = leBuildNaturalLabels($bonusRangeMin, $bonusRangeMax);
$bonusChartValues = [];
...
if ($drawDate !== '') {
    foreach ($latestMainBalls as $ball) {
        $prevDate = leGetPreviousOccurrenceDate(
            $db, (string) $dbCol, $gameId, $drawDate, $ball, false, $bonusBallCol, $mainBallCols
        );
    ...
    $prevBonusDate = leGetPreviousOccurrenceDate(
        $db, (string) $dbCol, $gameId, $drawDate, $pb, true, $bonusBallCol, $mainBallCols
    );
    $bonusDrawsAgo = leGetDrawingsSinceDate($db, (string) $dbCol, $gameId, $prevBonusDate, $drawDate);

    $drawHistoryRows[] = [
        'label'    => $bonusShort . ' (' . lePad2($pb) . ')',
 ...
 * --------------------------------------------------------------------------
 */
$jsonLdGameName = (isset($stateName, $gName) && $stateName !== '' && $gName !== '')
    ? $stateName . ' ' . $gName
    : 'Lottery';
...
$jsonLdWebPage = [
    ...
    '@type'       => 'WebPage',
    'name'        => $jsonLdGameName . ' Results Intelligence | LottoExpert.net',
    'description' => $jsonLdGameName . ' draw frequency, recency, active and quiet numbers, and full historical analysis. Transparent data-driven lottery intelligence.',
...
$jsonLdDataset = [
    '@context'    => 'https://schema.org',
    '@type'       => 'Dataset',
    'name'        => $jsonLdGameName . ' Draw Frequency Dataset',
    'description' => 'Historical draw frequency data for ' . $jsonLdGameName . '. Includes per-number occurrence counts, recency, and window-based analysis across main numbers (01–35) and Bonus Ball (01–25).',
    'url'         => $canonicalNoQuery,
];
?>
<script type="application/ld+json">
<?php echo json_encode($jsonLdWebPage, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?>
</script>
<script type="application/ld+json">
<?php echo json_encode($jsonLdDataset, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?>
</script>
<style>
:root{
  --skai-blue:#1C66FF;
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=KY1">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=KY1">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=KY1">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?gameId=KY1">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="/all-lottery-heatmaps?gameId=KY1">Heatmap Analysis</a>
          </div>
        </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The first panel below shows the complete main-number distribution across all values 01&ndash;35. The second panel shows the complete Bonus Ball distribution across values 01&ndash;25 for the selected bonus window.
        </p>
      </div>
    </div>
      ...
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values 01&ndash;35 across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
        ...
        <div class="skai-grid">
          <div class="skai-card">
            <div class="skai-card-head skai-card-head--radiant">
              ...
              <span class="skai-card-sub">All values 01&ndash;25 across the last <?php echo (int) $nodCurrentBonus; ?> drawings</span>
            </div>
            <div class="skai-card-body">
              <div class="skai-chart-shell">
                <div class="skai-chart-frame">
                  <canvas id="bonusChart" aria-label="Bonus Ball distribution chart" role="img"></canvas>
                </div>
              </div>
            </div>
          </div>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use the tables for exact counts, recency, and personal tracking. Quick filters help narrow the view without losing full access to the complete dataset. Main numbers cover the range 01&ndash;35. Bonus Ball covers the range 01&ndash;25.
        </p>
      </div>
    </div>
    ...
    <div class="skai-controls">
      <form
        name="fqsearch"
        method="post"
        action="/all-us-lotteries/results-analysis?st=<?php echo htmlspecialchars((string) $stateAbrev, ENT_QUOTES, 'UTF-8'); ?>&amp;stn=<?php echo htmlspecialchars((string) $stateName, ENT_QUOTES, 'UTF-8'); ?>&amp;gm=<?php echo htmlspecialchars((string) $gName, ENT_QUOTES, 'UTF-8'); ?>#tables"
      >
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            Main numbers table
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;35</span>
          </div>
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--radiant">
            Bonus Ball table
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;25</span>
          </div>
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=KY1">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=KY1">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=KY1">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?gameId=KY1">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="/all-lottery-heatmaps?gameId=KY1">Heatmap Analysis</a>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing can provide useful context for reviewing draw history, but they should be treated as descriptive signals rather than guarantees. The purpose of this page is to make the recent behavior of the game easier to understand, compare, and carry into deeper SKAI analysis. Main numbers (01&ndash;35) and Bonus Ball (01&ndash;25) are analyzed separately; their pools do not overlap. Nothing on this page constitutes a prediction or a recommendation to purchase a ticket.
      </div>
    </div>
  </section>
...
(function () {
  ...
  function renderCharts() {
    ...
    var bonusCanvas     = document.getElementById('bonusChart');
    ...
    if (bonusCanvas && chartData.bonusLabels.length) {
  ...
  function bindTrackers() {
    ...
    function bindGroup(selector, wrap, chipClass, emptyText) {
      ...
      for (var i = 0; i < inputs.length; i++) {
        inputs[i].addEventListener('change', (function (s, w, c, e) {
          return function () {
            renderTracked(s, w, c, e);
          };
        }(selector, wrap, chipClass, emptyText)));
      }
  ...
  function bindFilters() {
    ...
    for (var k = 0; k < buttons.length; k++) {
      buttons[k].addEventListener('click', (function (btn) {
        return function () {
